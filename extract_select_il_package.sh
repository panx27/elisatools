...
if [ -n "$KEY" ]; then
    $SCRIPTDIR/one_button_lrlp.py --swap --lexversion il3 --evalil -t $TARBALL -l $LANGUAGE -r $DST -k $KEY -S set0 --ruby /Users/jonmay/.rvm/rubies/ruby-2.3.0/bin/ruby &> $DST/$LANGUAGE/one_button_lrlp.err
else
    $SCRIPTDIR/one_button_lrlp.py --swap --lexversion il3 --evalil -t $TARBALL -l $LANGUAGE -r $DST --ruby /Users/jonmay/.rvm/rubies/ruby-2.3.0/bin/ruby &> $DST/$LANGUAGE/one_button_lrlp.err
fi
...
$SCRIPTDIR/subselect_data.py --allperseg $PREFIX -i $DST/$LANGUAGE/parallel -e filtered -l $LANGUAGE -s 10000 10000 20000 -c syscomb test dev -t $SCRIPTDIR/incidentvocab &> $DST/$LANGUAGE/subselect_data.err
