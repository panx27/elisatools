...
try $SCRIPTDIR/make_mono_release.py -r $ROOT/$LANGUAGE/mono/extracted -c `basename $ROOT/$LANGUAGE/mono/extracted/*.manifest` -l $LANGUAGE -p $ROOT/$LANGUAGE/psm.ann -a $ROOT/$LANGUAGE/entity.ann -o $ROOT/$LANGUAGE/elisa.$LANGUAGE.y1r1.v1.xml 2> $ROOT/$LANGUAGE/xml.err;
echo "Done."
